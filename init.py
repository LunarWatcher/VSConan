...
def run(buildFolder = "build"):
    ...
    if(hasVsCode):
        (includes, rawVsCodeConfig) = parseJson(buildFolder, hasVsCode)
    else:
        includes = parseJson(buildFolder, hasVsCode)

    if(includes is None or len(includes) == 0):
        raise Exception("Failed to read data, or data is empty. Make sure conan_ycm_flags.json exists in the selected build folder, and has content")
